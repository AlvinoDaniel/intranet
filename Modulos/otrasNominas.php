<?php 
  ...
  $fecha_nomina = $_POST['fecha'];
  $cadena = explode( "||" , $fecha_nomina);
    
  
  $fecha_reporte = $fecha_nomina ;
  $n_filas_fun = 0;
  $datos_fun = $objFuncionario->getFuncionario( $n_filas_person,$cedula_fun);

  if (!isset($fecha_nomina)){
    $fecha_nom  ="TO_DATE('01/01/1970','DD/MM/YY')";
    $tipoextra =  '000';
    
  }else{
    $fecha_nom  ="TO_DATE('". $cadena[0]."','DD/MM/YY')";
    $tipoextra = $cadena[1];
  }
   $n_filas_fecha = 0;
  $fechasEdoCuenta = $objgenerico->fechas_nomina_dif( $n_filas_fecha,$cedula_fun);
...
//  Información de los datos de cabecera del Estado de Cuenta
     $n_filas_person = 0;
   $datos_cabecera = $objgenerico->cabecera_dif( $n_filas_person,$cedula_fun,$fecha_nom,$tipoextra);
...
//  Información de detalle de los conceptos del estado de cuenta
  $n_filas = 0;
  $datos = $objgenerico->vista_dif( $n_filas,$cedula_fun,$fecha_nom,$tipoextra);
...
//      SUMA DE ASIGNACIONES 
  ...
  $datossueldo = $objsueldo->asignaciones_dif($cedula_fun,$fecha_nom,$tipoextra);
    $fijo = $datossueldo[0]['MONTO'];
  $fijo2= $datossueldo[0]['MONTO2'];
  
  $objdeducion = new cNomina();
  $datosdeducion = $objdeducion->deduciones_dif($cedula_fun,$fecha_nom,$tipoextra);
...
<div x-data="invoicePreview">
    ...
    <div class="panel" id="areaImprimir">
        <div class="flex flex-wrap justify-between gap-4 px-4">
            <div class="text-2xl font-semibold uppercase">Otras Nóminas</div>
            <div class="shrink-0">
                <form action="./index.php?item=otrasNominas" method="POST">
                      <select name="fecha" size="1" class="titulosmedianosAzulesSubRayado" id="fecha" onChange="this.form.submit()">
                        ...
                        <option value="<?php echo  $fechasEdoCuenta[$i]['FECHA'].'||'.$fechasEdoCuenta[$i]['CODIGO']; ?>" <?php if (!(strcmp($fechasEdoCuenta[$i]['FECHA'].'||'.$fechasEdoCuenta[$i]['CODIGO'], $fecha_reporte))) {echo "SELECTED";} ?>><?php echo  $fechasEdoCuenta[$i]['FECHA'].'    '.$fechasEdoCuenta[$i]['CODIGO'].'    '.$fechasEdoCuenta[$i]['DESCRIPCION'] ; ?></option>
                        <?php }?>
                      </select>
                    </form>
            </div>
        ...
        <!-- <div class="flex flex-col flex-wrap justify-between gap-6 lg:flex-row"> -->
            ...
            <div class="grid-cols-1 grid gap-6 sm:grid-cols-2" style="font-size: 12px">
