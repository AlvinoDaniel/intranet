...
//VARIABLES NECESARIAS PARA LA CONFIGURACIÒN  MODIFICAR Y COLOCAR LASPAGINAS  RESPECTIVAS
    ...
    $objsueldo = new cNomina();
    $datossueldo_homo = $objsueldo->asignaciones_homo($cedula_fun,$fecha_nom);
   
    $objdeducion = new cNomina();
    $datosdeducion_homo = $objdeducion->deduciones_homo($cedula_fun,$fecha_nom);
    
    $n_filas = 0;
    $datos = $objgenerico->vista_homo( $n_filas,$cedula_fun,$fecha_nom);
    $fijo=0;
    $fijo2=0;
    $deducion=0;
    $deducion2=0;
    $fijo = $datossueldo_homo[0]['MONTO'];
    $fijo2= $datossueldo_homo[0]['MONTO2'];
    $deducion = $datosdeducion_homo[0]['MONTO'];
    $deducion2 = $datosdeducion_homo[0]['MONTO2'];
?>
...
<div x-data="invoicePreview">
    ...
    <div class="panel" id="areaImprimir">
        <div class="flex flex-wrap justify-between gap-4 px-4">
            <div class="text-2xl font-semibold uppercase">Nómina Completa</div>
            <div class="shrink-0">
                <form action="./index.php?item=nominasCompletas" method="POST">
